...
import { ensureIds } from '../utils/id'
...
export function connectStateProps(nodePathMap: NodePathMap, props: PropsConfig): { [key:string]: any } {
  return Object.entries(props).reduce<{ [key:string]: any }>((memo, [key, value]) => {
    if (value === undefined) {
      return memo
    }
  ...
  return config.map<ReactNode>(child => {
    ...
    return (
      <LazyLoader
        ...
        type={child.type}
      >
        {child.children !== undefined && renderBlocks(host, child.children, nodePathMap)}
      </LazyLoader>
    )
  })
}
...
const BlockContainer: FC<BlockContainerProps> = ({
  ...
  const { connectedConfig, connections, nodePathMap } = createConnections(config)
